package com.yyy.dailycode.stock.getstockdata.diffchannel.unuse;
...
public class GetStockHisData {
	 ...
	 * @属性说明：json数据来源的url地址
	 **/
	private static final String URL = "http://gupiao.baidu.com/api/stocks/stockdaybar?from=pc&os_ver=1&cuid=xxx&vv=100&format=json&stock_code={0}&step=3&start=&count=1&fq_type=no";

	public static void main(String[] args) throws Exception {
//		loadDataOneConnection();
		String initURLStr,stockId="sz002820",stockCode="sz002820";
		initURLStr = initURLStr(stockId, CYCLETYPE);
		System.out.println(initURLStr);
	...
	public static void loadDataOneConnection() throws SQLException {
		...
		try {
			...
			for (StockItems si : listStockItems) {
				...
				stockCode = si.getStock_code();
				initURLStr = initURLStr(stockId, CYCLETYPE);
				loadDataFromBaiduToDbOneCon(initURLStr, stockCode, conn, ps, SQL_PREFIX);
			}
		} catch (Exception e) {
	...
	public static void loadDataFromBaiduToDbOneCon(String urlStr, String stockCode, Connection conn, PreparedStatement ps, String sqlPrefix)
			throws SQLException {
		URL url = null;
		InputStream is = null;
		...
		try {
			...
			String jsonStr = null;
			try {
				while ((jsonStr = br.readLine()) != null && jsonStr.contains(MASHDATA)) {
					JSONObject jsonObject = JSONObject.fromObject(jsonStr);
					JSONArray jsonArray = jsonObject.getJSONArray(MASHDATA);
					Collection<StockData> col = JSONArray.toCollection(jsonArray, StockData.class);
				...
				_log.error(LoadErrorProperties.getValue(ERROR_002, new Object[] { stockCode, count }), e);
			} catch (SQLException e) {
				_log.error(LoadErrorProperties.getValue(ERROR_003, new Object[] { stockCode, count }), e);
				conn.rollback();
			}catch (Exception e) {
				_log.error(LoadErrorProperties.getValue(ERROR_009, new Object[] { stockCode, count }), e);
				conn.rollback();
			}
		} catch (IOException e) {
			_log.error(LoadErrorProperties.getValue(ERROR_004, new Object[] { stockCode, urlStr }), e);
		} finally {
			try {
				if (is != null) {
					is.close();
				...
				_log.error(LoadErrorProperties.getValue(ERROR_005, new Object[] { stockCode, urlStr }), e);
			}
		}
	}
...
//					System.out.println(jsonStr);
	 ...
	 * @属性说明：错误代码009
	 **/
	private static final String ERROR_009 = "error_009";
}
